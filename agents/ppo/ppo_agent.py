...
class ActorCriticNetwork(nn.Module):
    """
    Actor-Critic network with opponent modeling.

    Architecture:
        1. OpponentEncoder processes competitor prices (50:500) → embedding (64,)
        2. Shared backbone processes full observation → features (256,)
        3. Opponent embedding concatenated with backbone features → (320,)
        4. Actor head: (320,) → logits (n_products, n_actions)
        5. Critic head: (320,) → scalar value

    The shared backbone ensures actor and critic both benefit from
    competitor-aware representations, while the opponent encoder
    provides an explicit competitive signal.
    """
    ...
    def __init__(self, obs_size: int, n_products: int, n_actions: int):
        ...
        combined_dim = 256 + opp_dim  # 320

        # actor and critic heads operate on combined representation
...
class PPOAgent(BaseRLAgent):
    """
    Proximal Policy Optimization agent with opponent modeling.

    Uses:
    - ActorCriticNetwork with OpponentEncoder for competitor-aware policy
    - GAE (Generalized Advantage Estimation)
    - Clipped surrogate objective
    - Entropy bonus for exploration
    - Multiple epochs of minibatch updates per rollout

    Assigned to: Target (profit_margin), Safeway (long_term_value),
                 Whole Foods (prestige_reward)
    """
    ...
    def __init__(
        ...
        self.logger.info(
            ...
            f"rollout={self.rollout_len} | opponent_encoder=True"
        )
    ...
    def _compute_gae(self, rewards, values, dones, next_value) -> np.ndarray:
        n          = len(rewards)
